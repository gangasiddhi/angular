...
export function createCrudReducer<T extends Entity>(
  actions: ReturnType<typeof createFeatureActions<T>>,
  initialState: FeatureState<T>,
) {
  return createReducer(
    ...
    on(actions.loadAllSuccess, (state, { data }) => ({
      ...state,
      loading: false,
      entities: data as T[],
    })),
    ...
    on(actions.loadByIdSuccess, (state, { data }) => ({
      ...state,
      loading: false,
      selected: data as T,
    })),
